...
box=read(G/'final_actual_boxes/numerical_state_box_check.json')
adj=read(G/'final_actual_anchors/actual_anchor_adjudication.json')
trace=read(OUT/'release_v4/runs/nominal/dynamic_trace_for_CAD.json')['samples']
bounds=read(OUT/'release_v4/runs/nominal/interval_joint_bounds.json')
report=read(OUT/'release_v4/runs/nominal/report.json')
...
floor=read('work/r20-walking-fix/gait/full_body_floor_check/final_v4_actual_nominal/index.json')
anchor_module=module(G/'adjudicate_actual_anchors.py','actual_anchor_independent_module')
...
assert N==box['interval_count']==len(bounds['intervals'])==len(trace)-1==8743
assert box['total_integration_steps']==bounds['total_integration_steps']==699386
...
assert steps==699386 and abs(trace[-1]['time_s']-report['simulated_duration_s'])<1e-10
...
ev=StaticEvaluator(OUT/'release_v4/verified_models/current_robot_contact4.xml',OUT/'release_v4/verified_models/model_contract_contact4.json')
