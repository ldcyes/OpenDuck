...
def build(n):
 src=OUT/'current_robot_r20.xml';cp=OUT/'model_contract_r20.json';c=json.loads(cp.read_text());root=E.parse(src).getroot();asset=root.find('asset');records=[]
 ...
 out=OUT/f'current_robot_r20_contact{n*n}.xml';E.indent(root);out.write_text(E.tostring(root,encoding='unicode')+'\n');m=mujoco.MjModel.from_xml_path(str(out));oldm=mujoco.MjModel.from_xml_path(str(src))
 ...
 c=deepcopy(c);c['contact_partition_diagnostic']=records;c['sources'][str(src.relative_to(ROOT))]=sha(src);c['sources'][str(cp.relative_to(ROOT))]=sha(cp);c['sources'][str(out.relative_to(ROOT))]=sha(out);c['sources'][str(Path(__file__).relative_to(ROOT))]=sha(Path(__file__));c['assumptions']+=['Ground contact uses an exact partition of the original sole prism because MuJoCo3.3.7 plane-mesh collider selects at most3 points per mesh, potentially two points on a diagonal. This tests discretization, not a friction or physical-material improvement.'];cpath=OUT/f'model_contract_r20_contact{n*n}.json';cpath.write_text(json.dumps(c,ensure_ascii=False,indent=2)+'\n');(OUT/f'contact_partition{n*n}_check.json').write_text(json.dumps(dict(status='GEOMETRY_PRESERVING_CONTACT_PARTITION_DIAGNOSTIC',model=str(out.relative_to(ROOT)),contract=str(cpath.relative_to(ROOT)),checks=records,unchanged_inertial_and_joint_and_torque_arrays=True,physical_approved=False),ensure_ascii=False,indent=2)+'\n');print(out)
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--n',type=int,default=2);build(p.parse_args().n)
