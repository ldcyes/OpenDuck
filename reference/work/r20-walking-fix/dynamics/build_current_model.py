...
delta_path=ROOT/'work/r20-walking-fix/head_mount/mass_inertia_delta.json';delta=json.loads(delta_path.read_text());old=json.loads(CONTRACT.read_text());sources=dict(old['sources']);sources[str(CONTRACT.relative_to(ROOT))]=sha(CONTRACT);sources[str(MODEL.relative_to(ROOT))]=sha(MODEL);sources[str(delta_path.relative_to(ROOT))]=sha(delta_path)
...
assert len(rows)==delta['after_mass_row_count'];mass=sum(r['mass_kg']for r in rows);assert abs(mass-delta['after_nominal_kg'])<1e-12
links={}
...
model_path=OUT/'current_robot_r20.xml';E.indent(root);model_path.write_text(E.tostring(root,encoding='unicode')+'\n');model=mujoco.MjModel.from_xml_path(str(model_path));assert abs(sum(model.body_mass)-mass)<1e-12
...
com=sum(r['mass_kg']*np.array(r['COM_HOME_m'])for r in rows)/mass;contract=deepcopy(old);contract.update(mass_kg=mass,COM_HOME_mm=(com*1000).tolist(),assembly_parts=557,mass_rows=len(rows),rows=rows,links=links,sources=sources,revision='R20_MASS_REALLOCATION_AND_CLAMP_REMOVAL',mass_delta_from_R19_kg=mass-old['mass_kg']);contract['assumptions']=old['assumptions']+[f'R20 removes two old clamp rows and allocates 28 explicit nominal fasteners from the preexisting head reserve. No double counting. The remaining reserve retains its unmeasured COM and m*r^2 estimate. There are now {len(rows)} rows for 557 CAD objects.']
contract['sources'][str(Path(__file__).relative_to(ROOT))]=sha(Path(__file__));contract['sources'][str(model_path.relative_to(ROOT))]=sha(model_path);contract_path=OUT/'model_contract_r20.json';contract_path.write_text(json.dumps(contract,ensure_ascii=False,indent=2)+'\n')
...
summary=dict(status='R20_MASS_REALLOCATION_COMPILED_AND_CHECKED',mass_kg=mass,COM_HOME_mm=(com*1000).tolist(),delta_kg=mass-old['mass_kg'],rows=len(rows),parts=557,unchanged_joint_and_contact_and_cap_arrays=True,COM_crosschecks=checks,physical_approved=False,sources={str(p.relative_to(ROOT)):sha(p)for p in [MODEL,CONTRACT,model_path,contract_path,delta_path,Path(__file__)]});(OUT/'model_update_check.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n');print(json.dumps(summary,ensure_ascii=False))
