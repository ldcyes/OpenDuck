...
GUARD=LegBatteryGuard();RES=json.loads((OUT/'reserved_endpoints.json').read_text());STANCE=RES['stance'];INIT=STANCE['initial_q_HOME_delta_deg'];BINIT=np.array(STANCE['initial_base_transform_m']);BINIT[:3,3]*=1000;TARGET0={l:np.array(t)for l,t in STANCE['foot_target_transforms_mm'].items()};L='ankle_left';R='ankle_right';CAP=.85;DTOL=np.deg2rad(1.3)
BOUNDS=[(-np.deg2rad(30),np.deg2rad(30))if 'yaw'in n else(-np.pi/4,np.pi/4)for n in LEGS]
source_left=RES['rows'][0]['endpoint'];source_right=next(r['endpoint']for r in RES['rows']if r['label']=='right_after_right_step_10mm')
ALL=[];LOG=[]
...
def evaluate(q,targets,support):
 ...
 return dict(q=q,T=T,B=B,eq=eq,static=st,rpy=rp,gaps=g)

def solve(prev,seed,targets,support,ctarget=None,single=False,fixed=None):
 xx=xq(prev);seedx=xq(seed);memo={}
 def ev(x):
  key=x.tobytes()
  if memo.get('key')!=key:memo.update(key=key,val=evaluate(qdict(x),targets,support))
  return memo['val']
 def eq(x):
 ...
 if fixed is not None:
  ...
  sol=minimize(lambda x:np.sum((x-seedx)**2)+.01*np.sum((x-xx)**2),xx,method='SLSQP',bounds=bb,constraints=[dict(type='eq',fun=eq),dict(type='ineq',fun=ine)],options=dict(maxiter=120,ftol=5e-11));x=sol.x;success=bool(sol.success);msg=sol.message;iterations=int(sol.nit)
 ...
 return e,dict(q_HOME_delta_deg=e['q'],base_transform_m=to_m(e['B']),support_link=support,foot_target_transforms_mm={l:t.tolist()for l,t in targets.items()},static_designated_support=e['static'],body_rpy_deg=np.rad2deg(e['rpy']).tolist(),leg_battery_gaps_mm=e['gaps'].tolist(),max_foot_target_point_error_mm=float(err),target_COM_world_mm=ctarget.tolist()if ctarget is not None else None,solver_success=success,solver_message=msg,iterations=iterations,numerically_feasible=valid,closure_residual=float(np.max(abs(eq(x)))),minimum_inequality=float(ine(x).min()),maximum_from_previous_joint_delta_deg=maxdq)

def save_partial():
 (OUT/'guarded_cycle_keypoints_v1.json').write_text(json.dumps(dict(status='BUILDING_GUARDED_TRUE_STEP_CYCLE',physical_approved=False,keypoints=ALL,log=LOG,stance=STANCE,guard_sources=GUARD.sources.entries),ensure_ascii=False,indent=2)+'\n')
...
qprev=INIT.copy();targets={l:t.copy()for l,t in TARGET0.items()};e,row=solve(qprev,qprev,targets,L,fixed=qprev);record(row,'initial_settle',0,[L,R],{L:.5,R:.5},hold=2.)

def transfer(goal,support,phase,load0,load1,n=80):
 global qprev
 initial=qprev.copy();e0=evaluate(qprev,targets,support);eg=evaluate(goal,targets,support);c0=np.array(e0['static']['COM_world_m'])*1000;c1=np.array(eg['static']['COM_world_m'])*1000
...
transfer(source_left['q_HOME_delta_deg'],L,'double_transfer_to_left',{L:.5,R:.5},{L:1.,R:0.},80);hold('pre_unload_right',L,{L:1.,R:0.});step(L,'right_step');hold('right_landing_still_unloaded',L,{L:1.,R:0.})
transfer(source_right['q_HOME_delta_deg'],R,'double_transfer_to_right',{L:1.,R:0.},{L:0.,R:1.},100);hold('pre_unload_left',R,{L:0.,R:1.});step(R,'left_step');hold('left_landing_still_unloaded',R,{L:0.,R:1.})
transfer(INIT,R,'double_recenter',{L:0.,R:1.},{L:.5,R:.5},80);hold('cycle_end_center',R,{L:.5,R:.5})
assert max(abs(qprev[n]-INIT[n])for n in names)==0
...
report=dict(status='GUARDED_TWO_TRUE_STEPS_EXACT_CYCLE_CANDIDATE_NOT_APPROVED',physical_approved=False,geometry_approved=False,dynamic_approved=False,keypoints=ALL,stance=STANCE,cycle_end_minus_start_q_max_deg=0,cycle_translation_mm=[10,0,0],swing_lift_mm=5,each_foot_step_mm=10,single_support_static_utilization_cap=CAP,leg_battery_sampled_gap_requirement_mm=2.3,number_numerical_guard_pairs=12,maximum_keypoint_joint_delta_deg=max(r['maximum_from_previous_joint_delta_deg']for r in ALL),guard_sources=GUARD.sources.entries,limits=['All actual 531 part pairs require independent review.','Static inverse dynamics and assigned contact load fractions do not prove dynamic balance.','No independent foot narrowing was prescribed; actual 5DOF geometry and feet remain unchanged.','Each pair of q knots will use a shared quintic time scalar, not a spline. Between-knot closure must be explicitly checked.'])
(OUT/'guarded_cycle_keypoints_v1.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print('CYCLE_COMPLETE',len(ALL),flush=True)
