...
import json,sys,hashlib,math,argparse,re
import numpy as np
import trimesh
from scipy.spatial import ConvexHull,QhullError
ROOT=Path(__file__).resolve().parents[3];HERE=Path(__file__).resolve().parent
ap=argparse.ArgumentParser();ap.add_argument('--trajectory',required=True);ap.add_argument('--label',required=True);ap.add_argument('--selection',required=True);ap.add_argument('--manifest',required=True);args=ap.parse_args();assert re.fullmatch("[a-z0-9_]+",args.label);OUT=HERE/args.label;OUT.mkdir(parents=True,exist_ok=False)
sys.path.insert(0,str(ROOT/'work/r18-leg-hip-covers/review'))
from motion_core import transforms
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
SOURCES={'work/r20-walking-fix/gait/full_body_floor_check/check_floor_final_selection.py':'9a25fd8c49514b4554a0cfbe43a933bc3c4b893e8710ac8bcf627c4dbd28581c'}
def read(p):
...
index=dict(status='SOURCE_HASH_VERIFIED_R21_REAL_VERTEX_FLOOR_CHECK',physical_approved=False,files=files,sources=SOURCES,summary=reports)
