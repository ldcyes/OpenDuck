...
CHECKPOINT=SOURCES.json('work/r21-reduced-sway/gait/cycle_resume_checkpoint_v5.json');ALL=CHECKPOINT['keypoints'];LOG=[];assert len(ALL)==284;SOURCES.entries.update(CHECKPOINT['sources'])
...
def solve(prev,seed,targets,support,ctarget=None,single=False,fixed=None):
 ...
 def objective(x):
  return np.sum((x-seedx)**2)+.01*np.sum((x-xx)**2)
...
def save_partial():
 (OUT/'reduced_cycle_keypoints_v5.json').write_text(json.dumps(dict(status='BUILDING_GUARDED_TRUE_STEP_CYCLE',physical_approved=False,keypoints=ALL,log=LOG,stance=STANCE,guard_sources=GUARD.sources.entries),ensure_ascii=False,indent=2)+'\n')
...
initial=qprev.copy();e0=evaluate(initial,targets,R);eg=evaluate(INIT,targets,R);c0=np.array(e0['static']['COM_world_m'])*1000;c1=np.array(eg['static']['COM_world_m'])*1000;start_u=ALL[-1]['phase_progress'];load0=ALL[-1]['desired_load_fraction_by_link'].copy()
for i in range(1,7):
 u=i/6;seed={n:(1-u)*initial[n]+u*INIT[n]for n in names};ct=(1-u)*c0+u*c1;e,row=solve(qprev,seed,targets,R,ctarget=ct,fixed=INIT if i==6 else None);row['subphase']='exact_ready_stance_closure';row['local_subphase_progress']=u;load={l:(1-u)*load0[l]+u*.5 for l in [L,R]};record(row,'double_recenter',start_u+(1-start_u)*u,[L,R],load);qprev=e['q'];save_partial()
hold('cycle_end_center',R,{L:.5,R:.5})
...
(OUT/'reduced_cycle_keypoints_v5.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print('CYCLE_COMPLETE',len(ALL),flush=True)
