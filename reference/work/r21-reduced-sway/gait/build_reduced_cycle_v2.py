...
CHECKPOINT=SOURCES.json('work/r21-reduced-sway/gait/cycle_resume_checkpoint_v2.json');ALL=CHECKPOINT['keypoints'];LOG=[];assert len(ALL)==21;SOURCES.entries.update(CHECKPOINT['sources'])
...
def save_partial():
 (OUT/'reduced_cycle_keypoints_v2.json').write_text(json.dumps(dict(status='BUILDING_GUARDED_TRUE_STEP_CYCLE',physical_approved=False,keypoints=ALL,log=LOG,stance=STANCE,guard_sources=GUARD.sources.entries),ensure_ascii=False,indent=2)+'\n')
...
qprev=ALL[-1]['q_HOME_delta_deg'].copy();targets={l:t.copy()for l,t in TARGET0.items()}
...
(OUT/'reduced_cycle_keypoints_v2.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print('CYCLE_COMPLETE',len(ALL),flush=True)
