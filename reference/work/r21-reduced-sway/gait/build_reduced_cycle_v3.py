...
CHECKPOINT=SOURCES.json('work/r21-reduced-sway/gait/cycle_resume_checkpoint_v3.json');ALL=CHECKPOINT['keypoints'];LOG=[];assert len(ALL)==129;SOURCES.entries.update(CHECKPOINT['sources'])
...
def save_partial():
 (OUT/'reduced_cycle_keypoints_v3.json').write_text(json.dumps(dict(status='BUILDING_GUARDED_TRUE_STEP_CYCLE',physical_approved=False,keypoints=ALL,log=LOG,stance=STANCE,guard_sources=GUARD.sources.entries),ensure_ascii=False,indent=2)+'\n')
...
adaptive_transfer(source_right['q_HOME_delta_deg'],R,'double_transfer_to_right',{L:1.,R:0.},{L:0.,R:1.},50,initial=startq,start_u=.52);hold('pre_unload_left',R,{L:0.,R:1.});step(R,'left_step');hold('left_landing_still_unloaded',R,{L:0.,R:1.})
...
(OUT/'reduced_cycle_keypoints_v3.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print('CYCLE_COMPLETE',len(ALL),flush=True)
