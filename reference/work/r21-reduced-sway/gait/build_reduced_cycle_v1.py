...
def solve(prev,seed,targets,support,ctarget=None,single=False,fixed=None):
 ...
 if fixed is not None:
  ...
  sol=minimize(objective,xx,method='SLSQP',bounds=bb,constraints=[dict(type='eq',fun=eq),dict(type='ineq',fun=ine)],options=dict(maxiter=120,ftol=5e-11));x=sol.x;success=bool(sol.success);msg=sol.message;iterations=int(sol.nit)
...
def save_partial():
 (OUT/'reduced_cycle_keypoints_v1.json').write_text(json.dumps(dict(status='BUILDING_GUARDED_TRUE_STEP_CYCLE',physical_approved=False,keypoints=ALL,log=LOG,stance=STANCE,guard_sources=GUARD.sources.entries),ensure_ascii=False,indent=2)+'\n')
...
transfer(source_left['q_HOME_delta_deg'],L,'double_transfer_to_left',{L:.5,R:.5},{L:1.,R:0.},40);hold('pre_unload_right',L,{L:1.,R:0.});step(L,'right_step');hold('right_landing_still_unloaded',L,{L:1.,R:0.})
transfer(source_right['q_HOME_delta_deg'],R,'double_transfer_to_right',{L:1.,R:0.},{L:0.,R:1.},50);hold('pre_unload_left',R,{L:0.,R:1.});step(R,'left_step');hold('left_landing_still_unloaded',R,{L:0.,R:1.})
transfer(INIT,R,'double_recenter',{L:0.,R:1.},{L:.5,R:.5},40);hold('cycle_end_center',R,{L:.5,R:.5})
...
(OUT/'reduced_cycle_keypoints_v1.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print('CYCLE_COMPLETE',len(ALL),flush=True)
