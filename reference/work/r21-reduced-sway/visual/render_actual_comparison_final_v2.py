...
annotation=bpy.data.materials.new('R21_VIDEO_TEXT_PURE_BLACK_EMISSION');annotation.use_nodes=True
nodes=annotation.node_tree.nodes;nodes.clear();emission=nodes.new('ShaderNodeEmission');emission.inputs['Color'].default_value=(0,0,0,1);emission.inputs['Strength'].default_value=1
surface=nodes.new('ShaderNodeOutputMaterial');annotation.node_tree.links.new(emission.outputs[0],surface.inputs['Surface']);label.data.materials.clear();label.data.materials.append(annotation)
label.data.offset=.00025
...
for i,t in enumerate(times):
 ...
 label.data.body=body;label.data.size=.024 if a.scene_mode!='candidate_actual'else .015
