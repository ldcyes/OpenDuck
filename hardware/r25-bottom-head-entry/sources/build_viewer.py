"""Create the source-bound R25 model from retained R22 objects and new CAD."""
...
parser.add_argument('--selection', default='work/r25-bottom-head-entry/assembly_selection.json')
...
prior = read('work/r24-head-front/visual/build_index.json')
prior_check = read('work/r24-head-front/visual/readback.json')
prior_path = 'work/r24-head-front/visual/Microduck_R24_摄像头前盖.blend'
...
materials = {
    'shell': material('R25_外壳', (.87, .88, .85)),
    'rubber': material('R25_柔性护套', (.16, .19, .22)),
    'positive': material('R25_电源正极', (.55, .035, .028)),
    'ground': material('R25_地线', (.055, .065, .075)),
    'metal': material('R25_金属', (.46, .52, .57), .65),
    'insulator': material('R25_绝缘件', (.69, .55, .30)),
    'wire': material('R25_线束_示意色_实际按端号色表', (.055, .075, .095)),
    'black': material('R25_商用器件', (.06, .07, .09)),
    'lens': material('R25_光学镜头', (.025, .055, .07), .3),
    'pcb': material('R25_PCB', (.025, .26, .12)),
    'connector': material('R25_插头', (.81, .84, .81)),
}
...
world = bpy.data.worlds.new('R25_world')
...
def scene(name):
    ...
    s['scope'] = 'R25 static CAD integration; qualification is reported separately.'
...
main = scene('01_R25_完整装配')
bpy.context.window.scene = main
groups, objects, records = {}, {}, []
for part in selection['items']:
    name = part['name']
    domain = 'bottom_entry' if part.get('is_new') else None
    group_name = ('R25_' + domain) if domain else ('保留_' + part.get('category', 'R22'))
    ...
    for key, value in {'part_id': name, 'link_frame': part['link_frame'], 'source_mesh': part['mesh'],
                       ...
                       'r25_domain': domain or 'retained_R22',
                       'component_kind': part.get('kind', 'retained'),
                       'geometry_reference_only': part.get('reference_only', False),
                       'kinematics_mode': ('deformable_HOME_reference_only' if part['link_frame']=='MULTI_LINK_FLEX_HARNESS' else part.get('kinematics_mode', 'rigid_HOME')),
...
details = []
detail_specs = [('02_R25_头部摄像头前盖', lambda p: p['link_frame'] in ('jaw_soft', 'mouth_link'), (1, -.55, .25)), ('03_R25_头部底部走线', lambda p: (p['link_frame'] in ('jaw_soft', 'mouth_link', 'MULTI_LINK_FLEX_HARNESS') or p['name'].startswith('R23_neck_lower_')), (.6,-1,-.65))]
...
for title, predicate, direction in detail_specs:
    ...
    groups_detail, shown = {}, []
    for part in chosen:
        label = part.get('r25_domain', part.get('r22_domain', 'reference'))
...
for part in selection['service_items']:
    ...
    matrix[:3, 3] = np.array(part['t_mm']) / 1000 + shift
    obj.matrix_world = Matrix(matrix)
    obj.name = 'R25_SERVICE_' + part['name']
...
text = bpy.data.texts.new('00_R25_查看说明.txt')
text.write('R25 底部走线设计候选。场景01完整装配，场景02前盖，场景03底部走线。Outliner可选择和隐藏部件；摄像头为厂商尺寸包络，并非精确CAD。前盖/眼罩/四个壳体耳座新设计；镜头朝+X。3MF为打印主文件。装配、光学和运动检查的范围及未决事项见R25说明；不代表制造或实物验收通过。\n')
index = dict(status='R25_STATIC_MODEL_BUILT_READBACK_PENDING', main_scene=main.name,
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT / 'OpenDuck_R25_底部走线.blend'), compress=True)
if not args.no_render:
    for s in [main] + details:
        s.render.filepath = str(OUT / (s.name + '.png'))
        bpy.ops.render.render(write_still=True, scene=s.name)
print('R25_STATIC_MODEL_SAVED', len(records), flush=True)
